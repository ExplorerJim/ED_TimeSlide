...
namespace ED_TimeSlide
{
    public class ScanDataStorageDriver : IDisposable
    {
        ...
        private void ProcessQueueItemsInBatches()
        {
            ...
            using (var connection = new SqliteConnection(connString))
            {
                ...
                while (!_ingestionQueue.IsCompleted)
                {
                    ...
                    try
                    {
                        int elapsed = (int)(DateTime.UtcNow - lastFlushTime)
                            .TotalMilliseconds;

                        int remainingTimeout = Math.Max(
                            0,
                            BatchTimeoutMilliseconds - elapsed
                        );
        ...
        private long GetOrCreateCachedBody(SqliteConnection conn,SqliteTransaction trans,long systemDbId,EddnRecords rec)
        {
            ...
            #region Structural Invariant Orbit Ingestion Pass (IsRetrograde Evaluated)
            using (var cmd = conn.CreateCommand())
            {
                cmd.Transaction = trans;
                cmd.CommandText = @"
                ...
                // An orbit is mathematically retrograde if its absolute inclination angle exceeds 90 degrees
        ...
        public void EnqueueRecord(EddnRecords record)
        {
            ...
            else
            {
                try
                {
                    // Compute absolute retrograde vector orientation shifts before pushing onto task pools
                    if (record.Message != null)
                    {
                        // Orbit maps to retrograde if its physical absolute inclination exceeds 90 degrees
                        bool isRetro = Math.Abs(record.Message.OrbitalInclination) > 90.0;

                        // Pass calculated data flag down into transient structure models for processing runs
