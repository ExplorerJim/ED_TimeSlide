...
namespace ED_TimeSlide
{
    public class ScanDataStorageDriver : IDisposable
    {
        ...
        /// <summary> 
        /// Retrieves only System and Body string names for lightweight UI validation checks. 
        /// Bypasses bulky telemetry records for near-instant execution. 
        /// </summary>
        ...
        /// Extracts chronological telemetry lines into flat double arrays, performing on-the-fly 
        /// conversions from Unix seconds to native Excel OADate markers for plotter alignment loops. 
        /// </summary>
        ...
        /// <summary> 
        /// Reads total data counts from database table indexes for tracking performance metrics. 
        /// </summary>
        ...
        /// <summary>
        /// Builds the 5-table unified relational SQLite monolith. Leaves calculated constants 
        /// completely unconstrained (Nullable Nulls) to allow downstream staging-in-place updates.
        /// </summary>
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    try
                    {
                        using (var command = connection.CreateCommand())
                        {
                            ...
                            #region Table 4: ObitInfo Invariant Keplerian Constants Repository
                            command.CommandText = @"
        ...
        {
            // Cache Layer Hit: Skips the database engine read entirely
            if (_systemCache.TryGetValue(msg.SystemAddress, out long cachedId))
            {
                return cachedId;
            }

            // Cache Layer Miss: Write out to relational storage once
        ...
        private long GetOrCreateCachedBody(SqliteConnection conn,SqliteTransaction trans,long systemDbId,EddnRecords rec)
        {
            ...
            #region Write Master Body Record
            using (var cmd = conn.CreateCommand())
            {
                cmd.Transaction = trans;
                cmd.CommandText = @"
                ...
                // Map the native string class parameters directly to the schema
            ...
            #region Structural Invariant Orbit Ingestion Pass
            using (var cmd = conn.CreateCommand())
            {
                cmd.Transaction = trans;
                cmd.CommandText = @"
                    INSERT OR IGNORE INTO ObitInfo (
                        ...
                        OrbitalInclination, Periapsis, AnchorTimestamp_UnixSec, 
                        AnchorDistance_Ls, IsRetrograde, IsClimbingOutward
                    ) VALUES (
                        @bodyDbId, @sma, @ecc, @period, @incl, @peri, 
                        NULL, NULL, NULL, NULL
                    );";
            ...
            #region Parse and Map Parents JSON Hierarchy Array
            if (msg.Parents != null && msg.Parents.Length > 0)
            {
                // Walk down the array entries to reconstruct parent nodes
                for (int depth = 0; depth < msg.Parents.Length; depth++)
                {
                    var parentNode = msg.Parents[depth];
                    foreach (var pair in parentNode)
                    {
                        // Deduplicate or insert the parent row wrapper
                        long parentDbId = 0;
                        ...
                        // Save the depth link tracking values directly to the tree mapping index
                        using (var cmd = conn.CreateCommand())
                        {
                            cmd.Transaction = trans;
                            cmd.CommandText = @"
        ...
        /// <summary>
        /// Inserts raw telemetry rows directly into the DataPoints table. Maps timestamps
        /// explicitly onto the renamed Timestamp_UnixSec column layout.
        /// </summary>
        private void InsertDataPoint(SqliteConnection conn,SqliteTransaction trans,long bodyDbId,EddnRecords rec)
        {
            using (var cmd = conn.CreateCommand())
            {
                ...
                // Enforce deduplication natively at the database level
                cmd.CommandText = @"
